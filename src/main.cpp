...
MqttSensor mqtt_temperature_sensor(
  mqtt_client,
  "outside_temperature",
  "Außentemperatur",
  "temperature",
...
MqttBinarySensor mqtt_waterlevel_sensor(
  mqtt_client,
  "water_level_low",
  "Wasserstand gering",
  "{{ value_json.state }}"
...
void setup() {
  ...
  pump->switch_on(); //initial phase after start up
}
...
void loop() {
  ...
  if (led_display->run_startup_animation()){
    ...
    if (!pump->get_state() && pump->get_time_switched_off() + 5000 <= millis()){ //measure only when pump is off
    ...
    led_display->display_state(pump, waterlevel_sensor, temperature_sensor);
    ...
    if (time_serial_print_interval < millis()){ //every 10 seconds
      ...
      Serial.print("Pumpe: ");
      if (pump->get_state()){
        Serial.println("an (in " + String(pump->get_duration_until_off_s()) + " s aus)");
      }else{
        Serial.println("aus (in " + String(pump->get_duration_until_on_s()) + " s an)");
      }
      ...
      mqtt_temperature_sensor.set_state(temperature_sensor->get_temperature()); //bei sensor implementieren, nur änderungen senden! + max. anzahl anrefreshes
      mqtt_waterlevel_sensor.set_state(waterlevel_sensor->get_state() ? MqttBinarySensor::ON_STATE : MqttBinarySensor::OFF_STATE);
      
      //mqtt update - test values
      mqtt_pump.toggle();
